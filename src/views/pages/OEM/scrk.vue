<template>
    ...
    <div class="page_content">
        <div class="flex">
            <div class="buts">
                <el-button type="primary" @click="scancom">扫码完成</el-button>
                <el-button v-show="data.editstate == '1'" type="success" :icon="Check" circle
                    @click="scantijiao">提交</el-button>
                <el-popconfirm title="确定清空这次扫描数据?" @confirm="confirmEvent">
                    <template #reference>
                        <el-button v-show="data.editstate == '1'" type="danger" :icon="Delete" circle>清空</el-button>
                    </template>
                </el-popconfirm>
            </div>
            扫码日期:
            <el-date-picker v-model="data.searchParms.query" type="date" 
        ...
        <el-card shadow="never">
            <el-table :data="data.tableData" style="width: 100%" show-summary :summary-method="getSummaries" height="800">
                ...
                <el-table-column prop="bgsl03" label="数量" width="120" />
                <el-table-column prop="bgsl04" label="批号" width="120" />
                <el-table-column prop="bgsl06" label="统计日期" width="120" />
                <el-table-column prop="bgsl07" label="每箱数量" width="120" />
                <el-table-column prop="bgsl08" label="箱数" width="120" />
                <el-table-column prop="bgsl09" label="零散" width="120" />
...
import { scrkAddApi, scrktemListApi, scrktemClsApi, scrktemTodbApi, scrkListApi, scrkChangeapi } from '@/util/request'
...
const editForm = (formEl) => {
    scrkChangeapi([data.row, data.editstate]).then(res => {
...
const query_all = () => {
    scrkListApi(data.searchParms).then(res => {
        data.tableData = res
        data.editstate = '2'
    })
}
